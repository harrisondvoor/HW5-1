...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/dvoorHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    my_priority = StringField("What is the new priority of this item?", validators=[Required()])
...
def update(item):
    ...
    if form.validate_on_submit():
        krein = form.my_priority.data
        harrison = TodoItem.query.filter_by(description=item).first()
        harrison.priority = krein
        db.session.commit()
...
def delete(lst):
    ...
    db.session.commit()
    flash("Deleted List {}".format(obj.title))
    return redirect(url_for("all_lists"))
